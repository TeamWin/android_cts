...
public abstract class IsAppShownInWorkAppsStepAutomation implements Automation<Boolean> {

    private final String mAppLabel;

    public IsAppShownInWorkAppsStepAutomation(String appLabel) {
        mAppLabel = appLabel;
    }
    ...
    public Boolean automate() throws Throwable {
        ...
        UiObject match = TestApis.ui().device().findObject(
                new UiSelector().resourceId(
                        "com.google.android.apps.nexuslauncher:id/apps_list_view_work"))
                .getChild(new UiSelector().text(mAppLabel));

        Log.e("InteractiveAutomation", "Looking for app " + mAppLabel
                + " in work, found " + match + "(exists: " + match.exists() + ")");
        return match.exists();
    }
}
