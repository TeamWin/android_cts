...
open class AppOpMultiUserTestBase : BaseHostJUnit4Test() {
    private val RUNNER = "androidx.test.runner.AndroidJUnitRunner"
    private val DEFAULT_TIMEOUT = 10 * 60 * 1000L

    protected val TEST_PACKAGE = "android.appop.host.cts.app"
    ...
    fun setUp() {
        ...
        createdProfile = createProfileOutput.substring(createProfileOutput.lastIndexOf(" "))
                .trim().toInt()
        // Start profile so that activity manager updates profile mapping
        device.executeShellCommand("am start-user $createdProfile")
