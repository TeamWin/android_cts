...
public abstract class CtsVerifierTest extends BaseHostJUnit4Test {
    ...
    enum TestResult {
        NOT_RUN, PASSED, FAILED, ASSUMPTION_FAILED
    }
    ...
    public static void ensureCtsVerifierInstalled(TestInformation information) throws Exception {
        ...
        if (!information.getDevice().getInstalledPackageNames()
                .contains(CTS_VERIFIER_PACKAGE_NAME)) {
            sShouldUninstallCtsVerifier = true;
            information.getDevice().installPackage(
                    buildHelper.getTestFile("CtsVerifier.apk"),
                    /* reinstall= */ true, "--install-reason 4");
        }

        information.getDevice().executeShellCommand("appops set " + CTS_VERIFIER_PACKAGE_NAME
                + " android:read_device_identifiers allow");
        information.getDevice().executeShellCommand("appops set " + CTS_VERIFIER_PACKAGE_NAME
                + " MANAGE_EXTERNAL_STORAGE 0");

        for (String permission : PERMISSIONS) {
            information.getDevice().executeShellCommand("pm grant "
                    + CTS_VERIFIER_PACKAGE_NAME + " " + permission);
        }
    }
    ...
    void runTest(String testName, String... configRequirements) throws Exception {
        LogUtil.CLog.logAndDisplay(Log.LogLevel.INFO, "Running test " + testName);
        ...
        TestResult currentTestResult = getTestResult(testName);
        ...
        while (limit.isAfter(Instant.now())) {
            currentTestResult = getTestResult(testName);
            if (currentTestResult == TestResult.PASSED) {
                return; // We passed
            } else if (currentTestResult == TestResult.FAILED) {
                throw new AssertionFailedError("Failed in CtsVerifier");
    ...
    private void removeTestResult(String testName) throws Exception {
        getDevice().executeShellCommand("content delete --uri content://"
                + CTS_VERIFIER_PACKAGE_NAME + ".testresultsprovider/results --where 'testname=\""
                + CTS_VERIFIER_PACKAGE_NAME + testName + "\"'");
    }
    ...
    private TestResult getTestResult(String testName) {
        try {
            String resultString =
                    getDevice().executeShellCommand("content query --uri content://"
                            + CTS_VERIFIER_PACKAGE_NAME
                            + ".testresultsprovider/results --where 'testname=\""
                            + CTS_VERIFIER_PACKAGE_NAME + testName + "\"' --projection testresult");
            ...
            if (resultString.contains("testresult=1")) {
                ...
                LogUtil.CLog.logAndDisplay(Log.LogLevel.DEBUG,
                        "Unknown test result " + resultString);
                return TestResult.NOT_RUN;
            }
    ...
    void requireFeatures(String... features) throws Exception {
        for (String feature : features) {
            assumeTrue("Test requires feature "
                    + feature, getDevice().hasFeature(feature));
        }
    }
    ...
    void excludeFeatures(String... features) throws Exception {
        for (String feature : features) {
            assumeTrue("Test excludes feature " + feature,
                    !getDevice().hasFeature(feature));
        }
    }
}
