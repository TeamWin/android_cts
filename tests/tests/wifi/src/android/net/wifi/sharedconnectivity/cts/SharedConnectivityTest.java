...
public class SharedConnectivityTest {
    ...
    private static final TetherNetwork TEST_TETHER_NETWORK_1 = new TetherNetwork.Builder()
            .setDeviceId(1)
            .setDeviceInfo(new DeviceInfo.Builder()
                    .setDeviceType(DeviceInfo.DEVICE_TYPE_PHONE)
                    .setDeviceName("Matt's Phone")
                    .setModelName("Pixel 6a")
                    .setBatteryPercentage(80)
                    .setConnectionStrength(3)
                    .build())
            .setNetworkType(TetherNetwork.NETWORK_TYPE_CELLULAR)
            .setNetworkName("Google Fi")
            .setHotspotSsid("Instant Tether 12345")
            .setHotspotBssid("01:01:01:01:01:01")
            .setHotspotSecurityTypes(new int[]{WifiInfo.SECURITY_TYPE_PSK})
            .build();

    private static final TetherNetwork TEST_TETHER_NETWORK_2 = new TetherNetwork.Builder()
            .setDeviceId(2)
            .setDeviceInfo(new DeviceInfo.Builder()
                    .setDeviceType(DeviceInfo.DEVICE_TYPE_LAPTOP)
                    .setDeviceName("Matt's Laptop")
                    .setModelName("Pixelbook Go")
                    .setBatteryPercentage(30)
                    .setConnectionStrength(2)
                    .build())
            .setNetworkType(TetherNetwork.NETWORK_TYPE_WIFI)
    ...
    private static final KnownNetwork TEST_KNOWN_NETWORK_1 = new KnownNetwork.Builder()
            ...
            .setSecurityTypes(new int[]{WifiInfo.SECURITY_TYPE_WEP})
            .setDeviceInfo(new DeviceInfo.Builder()
                    .setDeviceType(DeviceInfo.DEVICE_TYPE_PHONE)
                    .setDeviceName("Isaac's Phone")
                    .setModelName("Pixel 7")
    ...
    private static final KnownNetwork TEST_KNOWN_NETWORK_2 = new KnownNetwork.Builder()
            .setNetworkSource(KnownNetwork.NETWORK_SOURCE_NEARBY_SELF)
            .setSsid("Cafe Wifi")
            .setSecurityTypes(new int[]{WifiInfo.SECURITY_TYPE_PSK})
            .setDeviceInfo(new DeviceInfo.Builder()
                    .setDeviceType(DeviceInfo.DEVICE_TYPE_PHONE)
                    .setDeviceName("Isaac's Work Phone")
                    .setModelName("Pixel 7 Pro")
    ...
    private static final TetherNetworkConnectionStatus TEST_TETHER_NETWORK_CONNECTION_STATUS =
            new TetherNetworkConnectionStatus.Builder()
                    .setStatus(TetherNetworkConnectionStatus.CONNECTION_STATUS_ENABLING_HOTSPOT)
                    .setTetherNetwork(TEST_TETHER_NETWORK_1)
                    .setExtras(Bundle.EMPTY)
                    .build();
    ...
    @Test
    public void connectTetherNetwork_withoutPermission_throwsSecurityException() throws Exception {
        Context context = InstrumentationRegistry.getInstrumentation().getContext();
        dropPermission();
        SharedConnectivityManager manager = getManager(context);
        TestSharedConnectivityService service = getService();
        ...
        assertThrows(SecurityException.class,
                () -> manager.connectTetherNetwork(TEST_TETHER_NETWORK_1));
    }

    @Test
    public void disconnectTetherNetwork_withoutPermission_throwsSecurityException()
            throws Exception {
        Context context = InstrumentationRegistry.getInstrumentation().getContext();
        dropPermission();
        SharedConnectivityManager manager = getManager(context);
        ...
        assertThrows(SecurityException.class,
                () -> manager.disconnectTetherNetwork(TEST_TETHER_NETWORK_1));
    }
    ...
    @Test
    public void setTetherNetworks_managerCallbackReceivedWithCorrectData() throws Exception {
        Context context = InstrumentationRegistry.getInstrumentation().getContext();
        grantPermission();
        try {
            SharedConnectivityManager manager = getManager(context);
            ...
            TestSharedConnectivityClientCallback callback =
                    new TestSharedConnectivityClientCallback() {
                        ...
                        public void onTetherNetworksUpdated(List<TetherNetwork> networks) {
                            super.onTetherNetworksUpdated(networks);
                            callbackLatch.countDown();
                        }
                    };
            manager.registerCallback(Runnable::run, callback);
            ...
            List<TetherNetwork> networks = Arrays.asList(TEST_TETHER_NETWORK_1,
                    TEST_TETHER_NETWORK_2);
            service.setTetherNetworks(networks);

            assertThat(callbackLatch.await(LATCH_TIMEOUT_SECS, TimeUnit.SECONDS)).isTrue();
            assertThat(callback.getTetherNetworksList()).containsExactlyElementsIn(networks);
        } finally {
            dropPermission();
        }
    }
    ...
    @Test
    public void updateTetherNetworkConnectionStatus_managerCallbackReceivedWithCorrectData()
            throws Exception {
        Context context = InstrumentationRegistry.getInstrumentation().getContext();
        grantPermission();
        try {
            ...
            TestSharedConnectivityClientCallback callback =
                    new TestSharedConnectivityClientCallback() {
                        ...
                        public void onTetherNetworkConnectionStatusChanged(
                            ...
                            super.onTetherNetworkConnectionStatusChanged(status);
                            callbackLatch.countDown();
                        }
                    };
            manager.registerCallback(Runnable::run, callback);
            ...
            service.updateTetherNetworkConnectionStatus(TEST_TETHER_NETWORK_CONNECTION_STATUS);

            assertThat(callbackLatch.await(LATCH_TIMEOUT_SECS, TimeUnit.SECONDS)).isTrue();
            assertThat(callback.getTetherNetworkConnectionStatus()).isEqualTo(
                    TEST_TETHER_NETWORK_CONNECTION_STATUS);
        } finally {
            dropPermission();
        }
    }
    ...
    @Test
    public void connectTetherNetwork_serviceCallbackReceivedWithCorrectData() throws Exception {
        Context context = InstrumentationRegistry.getInstrumentation().getContext();
        grantPermission();
        try {
            SharedConnectivityManager manager = getManager(context);
            ...
            for (int i = 0; service.getConnectTetherNetwork() == null && i < CHECK_RETRIES; i++) {
                Thread.sleep(CHECK_DELAY_MILLIS);
            }
            assertThat(service.getConnectTetherNetwork()).isNotNull();
            assertThat(service.getConnectTetherNetwork()).isEqualTo(TEST_TETHER_NETWORK_1);
        } finally {
            dropPermission();
        }
    }
    ...
    @Test
    public void disconnectTetherNetwork_serviceCallbackReceivedWithCorrectData() throws Exception {
        Context context = InstrumentationRegistry.getInstrumentation().getContext();
        grantPermission();
        try {
            SharedConnectivityManager manager = getManager(context);
            ...
            for (int i = 0; service.getDisconnectTetherNetwork() == null && i < CHECK_RETRIES;
                    i++) {
                Thread.sleep(CHECK_DELAY_MILLIS);
            }
            assertThat(service.getDisconnectTetherNetwork()).isNotNull();
            assertThat(service.getDisconnectTetherNetwork()).isEqualTo(TEST_TETHER_NETWORK_1);
        } finally {
            dropPermission();
        }
    }
