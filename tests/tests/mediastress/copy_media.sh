...
## script to install media stress test files manually

adbOptions=" "
resLabel=CtsMediaStressTestCases-1.0
srcDir="/tmp/$resLabel"
tgtDir="/sdcard/test"
...
if [ ! -f "/tmp/$resLabel.zip" ]; then
  wget "https://storage.googleapis.com/android_media/cts/tests/tests/mediastress/$resLabel.zip" -O /tmp/$resLabel.zip
fi
