...
public class SuplRrlpMessagesGenerator {
  ...
  public static byte[] generateSuplStartWcdmaMessage(int mcc, int mnc, int uc,
      @Nullable InetAddress ipAddress) throws UnknownHostException {
    ...
    version.setMajToNewInstance().setInteger(BigInteger.ONE);
   ...
   * @param uc cell identity
   */
  public static byte[] generateSuplPositionInitWcdmaMessage(SessionID sessionId,
      int mcc, int mnc, int uc) {

    ULP_PDU ulpPdu = new ULP_PDU();
    Version version = ulpPdu.setVersionToNewInstance();
    version.setMinToNewInstance().setInteger(BigInteger.ZERO);
    version.setMajToNewInstance().setInteger(BigInteger.valueOf(1));
    version.setServindToNewInstance().setInteger(BigInteger.ZERO);
    ulpPdu.setVersion(version);
