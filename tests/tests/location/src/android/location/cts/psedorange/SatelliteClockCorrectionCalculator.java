...
public class SatelliteClockCorrectionCalculator {
  ...
  /*
  Calculates Satellite Clock Error Rate but subtracting the Satellite Clock Correction of +- 0.5s
  This is better than differentiating, because both the orbital and relativity terms have
  nonlinearities that are not easily differentiable
   */
  public static SatClockCorrection calculateSatClockCorrWithErrorRateAndEccAnomAndTkIteratively(
      GpsEphemerisProto ephemerisProto, double receiverGpsTowAtTimeOfTransmission,
      double receiverGpsWeekAtTimeOfTrasnmission) throws Exception {
    SatClockCorrection satClockCorrectionPlus = calculateSatClockCorrAndEccAnomAndTkIteratively(
        ephemerisProto, receiverGpsTowAtTimeOfTransmission + 0.5,
    ...
    SatClockCorrection satClockCorrectionMinus = calculateSatClockCorrAndEccAnomAndTkIteratively(
        ephemerisProto, receiverGpsTowAtTimeOfTransmission - 0.5,
        receiverGpsWeekAtTimeOfTrasnmission);
    SatClockCorrection satClockCorrection = calculateSatClockCorrAndEccAnomAndTkIteratively(
        ephemerisProto, receiverGpsTowAtTimeOfTransmission,
        receiverGpsWeekAtTimeOfTrasnmission);
    return new SatClockCorrection(satClockCorrection.satelliteClockCorrectionMeters,
        satClockCorrectionPlus.satelliteClockCorrectionMeters
        - satClockCorrectionMinus.satelliteClockCorrectionMeters,
        satClockCorrection.eccentricAnomalyRadians, satClockCorrection.timeFromRefEpochSec);
  }
