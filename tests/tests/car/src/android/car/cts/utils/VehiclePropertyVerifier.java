...
public class VehiclePropertyVerifier<T> {
    ...
    public void enableAdasFeatureIfAdasStateProperty() {
        ...
        runWithShellPermissionIdentity(() -> {
            ...
            if (adasEnabledCarPropertyConfig == null || adasEnabledCarPropertyConfig
                    .getAreaIdConfig(GLOBAL_AREA_ID).getAccess()
    ...
    private <U> void storeCurrentValuesForProperty(CarPropertyConfig<U> carPropertyConfig) {
        ...
        if (areaIdToInitialValue.size() == 0) {
            return;
        }
    ...
    private static <U> SparseArray<U> getInitialValuesByAreaId(
        ...
        for (AreaIdConfig<?> areaIdConfig : carPropertyConfig.getAreaIdConfigs()) {
            if (areaIdConfig.getAccess() != CarPropertyConfig.VEHICLE_PROPERTY_ACCESS_READ_WRITE) {
                continue;
            }
    ...
    private void verifyAdasPropertyDisabled() {
        ...
        CarPropertyConfig<T> carPropertyConfig = getCarPropertyConfig();
        for (AreaIdConfig<?> areaIdConfig : carPropertyConfig.getAreaIdConfigs()) {
            if (areaIdConfig.getAccess() == CarPropertyConfig.VEHICLE_PROPERTY_ACCESS_WRITE) {
                continue;
            }
    ...
    private void verifyCarPropertyValueCallback() {
        CarPropertyConfig<T> carPropertyConfig = getCarPropertyConfig();
        if (carPropertyConfig.getAreaIdConfigs().get(0).getAccess()
                == CarPropertyConfig.VEHICLE_PROPERTY_ACCESS_WRITE) {
            verifyCallbackFails();
            return;
        }
