...
class ParamSensitivityTest(its_base_test.ItsBaseTest):
  ...
  def test_param_sensitivity(self):
    ...
    with its_session_utils.ItsSession(
      ...
      test_name_with_path = os.path.join(log_path, _NAME)
      ...
      sens_step = (sens_range[1] - sens_range[0]) / float(_NUM_STEPS-1)
      sensitivities = [
          int(sens_range[0] + i * sens_step) for i in range(_NUM_STEPS)]
    ...
    for i, means in enumerate([r_means, g_means, b_means]):
      for j in range(len(means)-1):
        if means[j+1] <= means[j]:
          raise AssertionError(f'{_COLORS[i]} cap {j} means[j+1]: '
